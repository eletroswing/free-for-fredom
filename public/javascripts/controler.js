...
if(!localStorage.getItem('last_redirect') ){
  localStorage.setItem('last_redirect', dayjs().toISOString())
}
else { 
  let date = dayjs(localStorage.getItem('last_redirect')).add(2, 'days')
...
function Ask(){
  alert('Olá. Devido a natureza de não anúncios, nós fazemos esse redirect a cada dois dias, espero que isso não prejudique sua experiencia!')
  window.location.href = "https://shope.ee/2px28Gak1g"
  localStorage.setItem('last_redirect', dayjs().toISOString())
}
